...
"""
from InfrastructuresPlanner.envs.decision_makers import decision_maker
import gym
import os
...
class infra_planner(gym.Env):
    def __init__(self, fixed_seed=0, infra_lvl =0):
        ...
        self.fixed_start = 0
        if self.fixed_start:
            self.starting_state = np.array([ [75, -0.15, -0.005], [0.001**2, 0.0001**2, 0.00001**2] ])
        ...
        # load external data
        data_dir ='./InfrastructuresPlanner/envs/data'
        filename = pjoin(data_dir, 'service_life.mat')
        self.CDF = sio.loadmat(filename)

        filename = pjoin(data_dir, 'Pretrained_SSMKR.mat')
        self.pretrained_ssm = sio.loadmat(filename)
        self.RegModel = RegressionModel()

        if self.load_bridge_data:
            filename = pjoin(data_dir, 'b00012.mat')
            self.bridge_data = sio.loadmat(filename)
        else: 
            self.bridge_data = []
        # Environment Seed
    ...
    def reset(self):
        ...
        critical_state = [ [50, -1.8], [50, -1.8], [50, -1.8], [45, -1.8], [45, -1.8], [45, -1.8] ]
        ...
        # interventions for Beams | Front Wall | Slabs | gaurdrail  | Wing Wall | Pavement
        # Material b12 
        material_ind = [8, 5, 1, 3, 6, 3]
        self.struc_attributes = [[material_ind[self.selected_cat], 50, 48.7664]]        # bridge attributes [Material, Age, Lattitude] b12: [[5, 50, 48.7664]] 
        ...
        for i in range(self.num_b):
            for j in range(self.num_c[i]):
                for k in range(0,self.num_e[i,j,0]):
                    ...
                    det_speed_Var = np.sqrt(AKr@init_var@np.transpose(AKr) + Var_w0**2)
                    if self.fixed_start:
                        sample_state[1] = det_speed_Ex
                    else:
                        sample_state[1] = np.random.normal(det_speed_Ex, det_speed_Var)[0,0]
    ...
    def step(self, action): # action is a scalar\
        ...
        return observation.astype(self.observation_space.dtype), reward, done, info
    """ Environment Actions ########################################################################################################
    ...
    def elem_action(self, action):
        reward = 0
        self.cc = self.selected_cat # only for training purposes, must remove later
        # prepare category state
        if self.element_lvl:
    ...
    def test_env(self, episodes_num):#, val):
        ...
        while ep < episodes_num:
            ...
            for k in range(total_years):
                ...
                goal = self.decision_maker(state, self.cc)#, val)
                if k == 0:
                    Q_val = self.decision_maker(state, self.cc, 1)
                    q_vals.append(Q_val)
                state, reward, _, step_info = self.step(goal)
                episode_reward += (reward/20) * 0.97**(k)
                step_goal += goal
    ...
    """  
    def decision_maker(self, state, cc = 0, return_q = 0):#, val):
        if self.discrete_actions:
            if self.element_lvl:
            ...
            if self.element_lvl:
                goal = self.agent_one_element(state, cc, return_q)
