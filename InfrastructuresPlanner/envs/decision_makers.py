import numpy as np
import os
from InfrastructuresPlanner.envs.network import policy_net
from InfrastructuresPlanner.envs.network import dueling_policy_net
try:
    import torch
...
class decision_maker:
    def __init__(self, int_Ex, include_budget, shutdown_cond, discrete_actions, b_min_functional):
        ...
        self.no_agent = pre_trained_models
        #['params-hrl-infra_env-it(100000)-[28, 17].tar']#,'params-hrl-infra_env-it(100000)-[22, 15].tar'
        # Beams | Front Wall | Slabs | gaurdrail  | Wing Wall | Pavement
        if self.no_agent:
            self.file_names = ''
            self.filenames_bridge = ''
        else:
    ...
    def agent_element_lvl_discrete(self, state, cc = 0):
        if self.file_names != '' and pre_trained_models:
    ...
    def agent_one_element_lvl(self, state, cc = 0, return_Qval = 0):
        if self.discrete_actions:
            ...
            if self.file_names != '' and pre_trained_models:
                goal = self.evaluate_agent(state, cc)
                if return_Qval:
                    goal = self.evaluate_agent_Qval(state, cc)
            else:
                if state[0] <= self.shutdown_cond:
                    goal = (100-state[0])/75
    ...
    def evaluate_agent_Qval(self, state, cc):
        if cc == 0:
            Qval = torch.max(self.policy_nn_beams(torch.FloatTensor(state).to(device)), dim = 1)[0].cpu().detach().numpy()
        elif cc == 1:
            Qval = torch.max(self.policy_nn_frontwall(torch.FloatTensor(state).to(device)), dim = 1)[0].cpu().detach().numpy()
        elif cc == 2:
            Qval = torch.max(self.policy_nn_slabs(torch.FloatTensor(state).to(device)), dim = 1)[0].cpu().detach().numpy()
        elif cc == 3:
            Qval = torch.max(self.policy_nn_gaurdrail(torch.FloatTensor(state).to(device)), dim = 1)[0].cpu().detach().numpy()
        elif cc == 4:
            Qval = torch.max(self.policy_nn_wingwall(torch.FloatTensor(state).to(device)), dim = 1)[0].cpu().detach().numpy()
        elif cc == 5:
            Qval = torch.max(self.policy_nn_pavement(torch.FloatTensor(state).to(device)), dim = 1)[0].cpu().detach().numpy()
    ...
    def plot_policy(self, save_policy = 0, filename = ''):
        # load policy 
        if filename != '':
            file_path = []
            self.policy_nn_beams = policy_net(action_size=5, input_size=2).to(device)
            file_path.append(os.path.join(".", "model", filename))
            self.policy_nn_beams.load_state_dict(torch.load(file_path[0])['policy_net'])
        #self.load_agent(self.file_names)
        ...
        plt.xlabel('Condition $\tilde{x}_t$')
        plt.ylabel('Speed $\tilde{\dot{x}}_t$')
        plt.title('Pavement')
        ...
        if save_policy:
            if filename != '':
                file_path = []
                file_path.append(os.path.join(".", filename + ".png"))
                plt.savefig(file_path[0], bbox_inches='tight' )
            else:
                plt.savefig('policies/policy_map_slabs_dueling64.pgf',bbox_inches='tight')
        else:
            plt.show()